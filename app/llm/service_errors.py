...
def check_response(response: requests.Response, *args, **kwargs):
    ...
    provider = {"openrouter.ai": "OpenRouter", "api.openai.com": "OpenAI", "api.jina.ai": "Jina", "api.groq.com": "Groq", "api.deepseek.com": "DeepSeek", "generativelanguage.googleapis.com": "Gemini"}.get(
    ...
    if status in (401, 403):
        ...
        elif any(x in detail for x in ("per-day", "per day", "daily limit", "daily quota", "requests/day", "tokens/day", "tpd", "rpd", "quota_exceeded", "quota exhausted")):
            kind = "quota_exhausted"
        elif any(x in detail for x in ("per-minute", "per minute", "requests/minute", "tokens/minute", "rpm", "tpm")):
            kind = "rate_limited"
        else:
            kind = "limit_unknown"
